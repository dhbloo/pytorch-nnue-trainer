    ...
    def __init__(self, in_dim, out_dim, expand_ratio):
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                Conv2dBlock(
                    in_dim, in_dim, ks=3, st=1, padding=1, groups=in_dim, bias=False, norm="bn", activation="relu"
                ),  # dw
                Conv2dBlock(in_dim, out_dim, ks=1, st=1, padding=0, bias=False, norm="bn"),  # pw-linear
            )
        else:
            self.conv = nn.Sequential(
                Conv2dBlock(in_dim, hidden_dim, ks=1, st=1, padding=0, bias=False, norm="bn", activation="relu"),  # pw
                Conv2dBlock(
                    hidden_dim,
                    hidden_dim,
                    ks=3,
                ...
                Conv2dBlock(hidden_dim, out_dim, ks=1, st=1, padding=0, bias=False, norm="bn"),  # pw-linear
            )
...
class MobileNetV1(nn.Module):
    def __init__(self, num_blocks, dim_feature, head_type="v0", input_type="basic"):
        ...
        self.conv_input = Conv2dBlock(
            self.input_plane.dim_plane, dim_feature, ks=3, st=1, padding=1, activation="lrelu"
        )
        self.conv_trunk = nn.Sequential(*[depthwise_conv_block(dim_feature, dim_feature) for i in range(num_blocks)])
...
class MobileNetV2(nn.Module):
    def __init__(self, num_blocks, dim_feature, expand_ratio=6, head_type="v0", input_type="basic"):
        super().__init__()
