...
@MODELS.register('mixs1')
class MixS1Net(nn.Module):
    def __init__(self,
                 dim_middle=128,
                 dim_middle_square=256,
                 dim_feature=64,
    ...
    def get_feature(self, data, inv_side=False):
        ...
        # concat feature from depth-wise conv and direct feature
        feature = torch.cat([feat_dwconv, feat_direct], dim=1)  # [B, dim_feature, H, W]

        # get 4x4 mapping feature
        feat4x4 = self.mapping4x4(input_plane)  # [B, dim_feature_square, H-3, W-3]
        feat4x4 = fake_quant(feat4x4, scale=128, num_bits=8)  # int8, scale=128, [-1,127/128]
        # feat4x4 = fake_quant(torch.clamp(feat4x4, min=-1, max=7 / 8), scale=8, num_bits=4)  # int4, scale=8, [-1,7/8]
        
        return feature, feat4x4

    def forward(self, data):
        dim_policy = self.model_size[-3]

        # get feature from single side
        feature, feature4x4 = self.get_feature(data, False)  # [B, dim_feature, H, W], [B, dim_feature_square, H-3, W-3]

        # value feature accumulator
        feature_sum = torch.sum(feature, dim=(2, 3))  # [B, dim_feature]
        feature_sum = fake_quant(feature_sum / 256, scale=128, num_bits=32, floor=True)  # srai 8
        feature4x4_sum = torch.sum(feature4x4, dim=(2, 3))  # [B, dim_feature_square]
        feature4x4_sum = fake_quant(feature4x4_sum / 128, scale=128, num_bits=32, floor=True)  # srai 7
        feature_sum = torch.cat([feature_sum, feature4x4_sum], dim=1)  # [B, dim_feature + dim_feature_square]
