...
class LinearModel(nn.Module):
    def __init__(self, pcode_dim=2380, has_draw_value=True, two_side=False, fixed_side_input=False):
        super().__init__()
        ...
        self.table = nn.Embedding(
            num_embeddings=(2 if two_side else 1) * pcode_dim, embedding_dim=2 + (1 if self.has_draw_value else 0)
        )
    ...
    def forward(self, data):
        ...
        value_map = torch.masked_fill(value_map, non_empty_mask.unsqueeze(4), 0)  # [B, 2, H, W, 1 or 2]
        ...
        if self.fixed_side_input:
            ...
            policy_self = torch.where(flip_cond[:, None, None], policy_map[:, 0], policy_map[:, 1])  # [B, H, W]
            policy_oppo = torch.where(flip_cond[:, None, None], policy_map[:, 1], policy_map[:, 0])  # [B, H, W]
        else:
