...
def get_sample_data(datas, dataset_type, dataset_args, dataloader_args, batch_size=1, train_datas=None, **kwargs):
    ...
    loader = build_data_loader(dataset, batch_size=batch_size, num_workers=1, shuffle=False, **dataloader_args)
...
def export_onnx(output, model, export_args, **kwargs):
    ...
    print(
        f"Onnx model (ver={version_number}, rules={supported_rules}, boardsizes={supported_boardsizes}) has been written to {output}."
    )
...
def export_serialization(output, output_type, model_type, model, export_args, use_cpu, no_header=False, **kwargs):
    ...
    def open_output(output):
        ...
        if output_type == "raw":
            ...
            raise ValueError(f"Unsupported serialization output {output_type}")
    ...
    with open_output(output) as f:
        ...
        if serializer.needs_header and not no_header:
            ...
            print(
                f"write header: magic = {hex(MAGIC)}, arch_hash = {hex(arch_hash)}, "
