...
def sgf_to_bin():
    ...
    parser.add_argument(
        "--rule", default="freestyle", choices=["freestyle", "standard", "renju"], help="Rule of this dataset"
    )
    parser.add_argument(
        "--not_compressed", action="store_true", default=False, help="Input is not compressed with lz4"
    )
    parser.add_argument(
        "--no_output_compress", action="store_true", default=False, help="Do not compress output with lz4"
    )
    args = parser.parse_args()

    if args.rule == "freestyle":
        rule = Rule.FREESTYLE
    elif args.rule == "standard":
        rule = Rule.STANDARD
    elif args.rule == "renju":
        rule = Rule.RENJU
    print(f"rule: {rule}")
